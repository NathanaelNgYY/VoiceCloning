...
test('router resolves generated inference chunk route', () => {
  assert.deepEqual(
    findRoute('GET', '/api/inference/chunk/abc-123/2'),
    {
      ...
      methods: ['GET', 'POST'],
      pattern: /^\/api\/inference(?:\/(?:generate|regenerate-chunk|delete-chunk|scan-oov|result\/[A-Za-z0-9-]+|chunk\/[A-Za-z0-9-]+\/\d+|chunk-preview\/[A-Za-z0-9-]+\/\d+|cancel|current|status|start|stop))?\/?$/u,
