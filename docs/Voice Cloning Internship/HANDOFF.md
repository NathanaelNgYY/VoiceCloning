...
- Repository scale-out is +60% after any one-minute period with a rejection; the live
  alarm still uses zero capacity plus rejects because AWS credentials expired before
  apply. Idle scale-in is -1 after 15 quiet minutes; floor 1 outside event mode.
