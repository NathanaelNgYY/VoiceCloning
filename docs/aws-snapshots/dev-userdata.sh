#!/bin/bash
# Dev instance first-boot config (runs as root). Idempotent-ish; logs to /var/log/dev-bootstrap.log
...
# 1) point workers at the dev S3 prefix + dev CORS
...
# drop hardcoded Environment= lines from the unit, add EnvironmentFile
