...
- ~~**Content is not GI-specific.**~~ **RETRACTED 2026-07-20 — this was wrong.** The claim was made from the filename without reading the file. `lib/chatbotSystemPrompt.js`'s `DEFAULT_CHATBOT_SYSTEM_PROMPT` is already a 992-line GI-bleeding teaching prompt: nine knowledge-base sections (core concepts, upper vs lower, Forrest classification, endoscopy timing, hemostatic treatments, Hemospray, PPIs post-endoscopy, *H. pylori*, aspirin vs clopidogrel), nine pre-written student answers, plus TTS/prosody and pronunciation sections specific to the GPT-SoVITS pipeline. `useGiChatEngine` calls `resolveChatbotSystemPrompt()`, so the gi build gets all of it. The source repo's `giBleedingInstructions.js` is 49 lines whose five bullets are a strict subset of the nine sections here — porting it would be a downgrade and would lose the TTS-shaping guidance. **No prompt work is needed.**
