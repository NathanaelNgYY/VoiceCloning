...
test('Live Full expands compact chemical formulas to unambiguous element names', () => {
  ...
  assert.match(result, /carbon six hydrogen twelve oxygen six/u);
  assert.match(result, /hydrogen two oxygen two/u);
  assert.match(result, /open parenthesis carbon hydrogen two oxygen close parenthesis en/u);
  assert.match(result, /carbon oxygen oxygen hydrogen/u);
...
test('Live Full recognizes element-symbol casing without treating normal capitals as formulas', () => {
  ...
  assert.match(result, /sodium chlorine/u);
  assert.match(result, /hydrogen two oxygen/u);
  assert.match(result, /iron two oxygen three/u);
...
test('Live Full speaks larger subscripts and numeric group counts without changing invalid symbols', () => {
  ...
  assert.match(result, /carbon twenty hydrogen one hundred one oxygen one zero zero zero/u);
  assert.match(result, /open parenthesis carbon hydrogen two oxygen close parenthesis six/u);
