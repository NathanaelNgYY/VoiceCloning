...
foreach ($hostInfo in $hosts) {
  ...
  $command = "set -e; cd /home/ubuntu/VoiceCloning; aws s3 cp $BundleUri /tmp/vcs.bundle --region ap-southeast-1 --only-show-errors; sudo -u ubuntu git fetch /tmp/vcs.bundle HEAD; sudo -u ubuntu git checkout -B $($hostInfo.Branch) FETCH_HEAD; sudo -u ubuntu npm --prefix gpu-worker ci --omit=dev; sudo -u ubuntu npm --prefix gpu-inference-worker ci --omit=dev; sudo -u ubuntu npm --prefix live-gateway ci --omit=dev; $setup; systemctl restart gpu-worker gpu-inference-worker voice-live-gateway; sleep 12; test `$(sudo -u ubuntu git rev-parse HEAD) = $Commit"
