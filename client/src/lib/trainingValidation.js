...
export function validateTrainingStart({
  ...
  if (!isIntegerInRange(sovitsEpochs, 1, 50)) {
    errors.push('Voice epochs must be between 1 and 50.');
  }
  if (!isIntegerInRange(gptEpochs, 1, 50)) {
    errors.push('Language epochs must be between 1 and 50.');
  }
  if (!isIntegerInRange(sovitsSaveEvery, 1, 10)) {
    errors.push('Voice save interval must be between 1 and 10.');
  }
  if (!isIntegerInRange(gptSaveEvery, 1, 10)) {
    errors.push('Language save interval must be between 1 and 10.');
  }
