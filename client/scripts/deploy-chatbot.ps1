# Deploy the chatbot kiosk build to S3 + CloudFront so it reaches ALL users immediately.
# Usage: pwsh client/scripts/deploy-chatbot.ps1 [-SkipBuild]
...
$Dist = Join-Path $ClientDir 'dist-chatbot'

if (-not $SkipBuild) {
    Push-Location $ClientDir
    try {
        npm run build:chatbot
        if ($LASTEXITCODE -ne 0) { throw "build:chatbot failed" }
